use crate::validators::phase_1::errors::{Phase1Error, Phase1Warning, ValidationError, ValidationResult, ValidationWarning};
use crate::validators::phase_1::common::Value;
use crate::validators::phase_1::helpers::credential_to_bech32_reward_address;
use crate::validators::phase_1::value::MultiAsset;
use crate::validators::phase_1::ValidationInputContext;
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum RefundType {
    StakeDeregistration { amount: i128, index: u32, reward_address: String },
    DrepDeregistration { amount: i128, index: u32, drep_bech32: String },
}
...
impl<'a> BalanceValidator<'a> {
    ...
    pub fn validate(&self) -> ValidationResult {
        ...
        if let Some(treasury_value) = self.treasury_value {
            if self.validation_input_context.treasury_value != treasury_value {
                errors.push(ValidationError::new(Phase1Error::TreasuryValueMismatch { 
    ...
    fn validate_withdrawals_balance(&self) -> ValidationResult {
        ...
        for withdrawal in self.inputs.withdrawals.iter() {
            ...
            let account_context = self.validation_input_context.find_account_context(&reward_address);
        ...
        for deposit in self.outputs.deposits.iter() {
            match deposit {
                DepositType::StakeRegistration { amount, index } => {
                    let stake_key_deposit = self.validation_input_context.protocol_parameters.stake_key_deposit;
                    if *amount != stake_key_deposit as i128 {
                        errors.push(ValidationError::new(Phase1Error::StakeRegistrationWrongDeposit {
                ...
                DepositType::DrepRegistration { amount, index } => {
                    let drep_deposit = self.validation_input_context.protocol_parameters.drep_deposit;
                    if *amount != drep_deposit as i128 {
                        errors.push(ValidationError::new(Phase1Error::DRepIncorrectDeposit {
                            supplied_deposit: *amount,
                ...
                DepositType::PoolRegistration { amount, index } => {
                    let stake_pool_deposit = self.validation_input_context.protocol_parameters.stake_pool_deposit;
                    if *amount != stake_pool_deposit as i128 {
                        errors.push(ValidationError::new(Phase1Error::PoolRegistrationWrongDeposit { 
                            supplied_deposit: *amount, 
                ...
                DepositType::VotingProposal { amount, index } => {
                    let proposal_deposit = self.validation_input_context.protocol_parameters.governance_action_deposit;
                    if *amount != proposal_deposit as i128 {
                        errors.push(ValidationError::new(Phase1Error::VotingProposalIncorrectDeposit {
                            supplied_deposit: *amount,
    ...
    fn validate_refunds(&self) -> ValidationResult {
        ...
        for refund in self.inputs.refunds.iter() {
            match refund {
                RefundType::StakeDeregistration { amount, index, reward_address } => {
                    if let Some(account_context) = self.validation_input_context.find_account_context(&reward_address) {
                        if let Some(payed_deposit) = account_context.payed_deposit {
                            if *amount != payed_deposit as i128 {
                                errors.push(ValidationError::new(Phase1Error::StakeDeregistrationWrongRefund {
                            ...
                            warnings.push(ValidationWarning::new(Phase1Warning::CannotCheckStakeDeregistrationRefund {
                                cert_index: *index,
                            }, format!("transaction.body.certs.{}", index)));
                        }
                    } else {
                ...
                RefundType::DrepDeregistration { amount, index, drep_bech32} => {
                    let drep_context = self.validation_input_context.find_drep_context(&drep_bech32);
                    if let Some(drep_context) = drep_context {
                        if let Some(payed_deposit) = drep_context.payed_deposit {
                            if *amount != payed_deposit as i128 {
                                errors.push(ValidationError::new(Phase1Error::DRepDeregistrationWrongRefund {
                                    supplied_refund: *amount,
                                    required_refund: payed_deposit as i128,
                                    cert_index: *index,
                                }, format!("transaction.body.certs.{}", index)));
                            ...
                            warnings.push(ValidationWarning::new(Phase1Warning::CannotCheckDRepDeregistrationRefund {
                                cert_index: *index,
                            }, format!("transaction.body.certs.{}", index)));
                        }
                    } else {
...
fn calculate_total_inputs(tx_body: &csl::TransactionBody, validation_input_context: &ValidationInputContext) -> Value {
    tx_body
        ...
        .into_iter()
        .map(|input| {
            let utxo = validation_input_context.find_utxo(input.transaction_id().to_hex(), input.index());
        ...
        match cert_type {
            csl::CertificateKind::StakeRegistration => {
                ...
                if let Some(explicit_deposit) = explicit_deposit {
                    ...
                    deposits.push(DepositType::StakeRegistration { 
                        amount: validation_input_context.protocol_parameters.stake_key_deposit as i128, 
                        index: i as u32 
                    });
                }
            }
            csl::CertificateKind::StakeDeregistration => {
                ...
                let payedout_address = credential_to_bech32_reward_address(&dereg_cert.stake_credential(), &validation_input_context.network_type);
                let account_context = validation_input_context.find_account_context(&payedout_address);
            ...
            csl::CertificateKind::PoolRegistration => {
                ...
                let pool_registration = validation_input_context.find_pool_context(&pool_operator.to_hex());
                if !(pool_registration.map_or(false, |pool_registration| pool_registration.is_registered)) {
                    deposits.push(DepositType::PoolRegistration { 
                        amount: validation_input_context.protocol_parameters.stake_pool_deposit as i128, 
                        index: i as u32 
                    });
                }
            },
