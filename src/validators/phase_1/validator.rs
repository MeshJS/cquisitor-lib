...
pub fn get_necessary_data_list(tx_hex: &str) -> Result<NecessaryInputData, String> {
    ...
    let network_type = match network_type.map(|n| n.kind()) {
        ...
        Some(csl::NetworkIdKind::Testnet) => NetworkType::Testnet,
    ...
    if let Some(withdrawals) = tx_body.withdrawals() {
        ...
        for i in 0..withdrawal_keys.len() {
            ...
            accounts.insert(reward_address.to_address().to_bech32(None).unwrap_or_else(|_| "".to_string()));
        }
    }
            ...
            match cert.kind() {
                csl::CertificateKind::StakeRegistration => {
                                if let Some(reg_cert) = cert.as_stake_registration() {
                                    ...
                                    let reward_address = credential_to_reward_address(&stake_credential, &network_type);
                ...
                csl::CertificateKind::CommitteeHotAuth => {
                                if let Some(committee_auth_cert) = cert.as_committee_hot_auth() {
                                    ...
                                    let local_credential = csl_credential_to_local_credential(&committee_cold_credential);
                                    committee_members.insert(local_credential);
                                }
                            }
        ...
        for i in 0..voting_proposals.len() {
            ...
            match gov_action.kind() {
                ...
                csl::GovernanceActionKind::HardForkInitiationAction => {
                    ...
                    let hard_fork_initiation_action = gov_action.as_hard_fork_initiation_action().unwrap();
                ...
                csl::GovernanceActionKind::TreasuryWithdrawalsAction => {
                    ...
                    let treasury_withdrawals_action = gov_action.as_treasury_withdrawals_action().unwrap();
    ...
    Ok(NecessaryInputData {
        ...
        committee_members: committee_members.into_iter().collect(),
    })
}

// Helper functions
fn credential_to_reward_address(credential: &csl::Credential, network_type: &NetworkType) -> String {
    ...
    reward_address.to_address().to_bech32(None).unwrap_or_else(|_| "".to_string())
}
